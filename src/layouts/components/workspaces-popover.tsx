...
export function WorkspacesPopover({ data = [], sx, ...other }: WorkspacesPopoverProps) {
  ...
  const renderLabel = (plan: string) => (
    <Button
      size="small"
      color={plan === 'Free' ? 'info' : 'success'}
      sx={{
        fontSize: '10px',
        background: (theme) =>
          plan === 'Free'
            ? varAlpha(theme.vars.palette.info.mainChannel, 0.2)
    ...
    <>
      ...
      >
        ...
        <Box
          ...
          sx={{ typography: 'body2', fontWeight: 'fontWeightSemiBold', fontSize: '12px' }}
        >
          {workspace?.name}
        ...
        >
          {data.map((option) => (
            ...
            >
              ...
              <Box component="span" sx={{ flexGrow: 1, fontSize: '12px' }}>
                {option.name}
              </Box>

              <Button
                size="small"
                color={option.plan === 'Free' ? 'info' : 'success'}
                sx={{ fontSize: '10px' }}
              >
                {option.promo_text}
              </Button>
